...
function Cwpp() {
  ...
  const [widgets, setWidgets] = useState(() => {
    const storedWidgets = JSON.parse(localStorage.getItem('data'))?.categories.find(category => category.name === 'CWPP Dasboard')?.widgets || [];
  ...
  useEffect(() => {
    ...
    const updatedCategories = data.categories.map(category => {
      if (category.name === 'CWPP Dashboard') {
  ...
  return (
    ...
    <h1 className='font-bold'>CWPP Dashboard</h1>
